...
public class AddBillItem extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        spinner_taxtes.setText("Taxs + " + ListPurchaseItem.adds_item.getProductServiceTaxes().size());
        try {
            name.setText(ListPurchaseItem.adds_item.getName());
            adds_item_name.setText(ListPurchaseItem.adds_item.getName());
            description.setText(ListPurchaseItem.adds_item.getDescription());
            if (!qty.getText().toString().equalsIgnoreCase(""))
                qty.setText(ListPurchaseItem.adds_item.getQuantity());
            else
                qty.setText("1");
            price.setText(ListPurchaseItem.adds_item.getPrice() + "");
            ...
            public void onClick(View v) {
                try {
                    if (!name.getText().toString().equalsIgnoreCase(""))
                        ListPurchaseItem.adds_item.setName(name.getText().toString());
                    if (!description.getText().toString().equalsIgnoreCase(""))
                        ListPurchaseItem.adds_item.setDescription(description.getText().toString());
                    if (!qty.getText().toString().equalsIgnoreCase(""))
                        ListPurchaseItem.adds_item.setQuantity(Integer.parseInt(qty.getText().toString()));
                    else
                        ListPurchaseItem.adds_item.setQuantity(1);
                    if (!price.getText().toString().equalsIgnoreCase(""))
                        ListPurchaseItem.adds_item.setPrice(Double.parseDouble(price.getText().toString()));
                    if (ListPurchaseItem.adds_item.getProductId() == 0){
                        ListPurchaseItem.adds_item.setProductId(ListPurchaseItem.adds_item.getId());}
                } catch (Exception e) {
                }
                lunch();
            }
        ...
        spinner_taxtes.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                showtaxs(UiUtil.makeCompatible(UiUtil.changeTaxResponse(taxReceivedList), ListPurchaseItem.adds_item.getProductServiceTaxes()));
            }
        });
        watcher();
        getTaxList();
            ...
            public void onResponse(Call<TaxResponseList> call, Response<TaxResponseList> response) {
                ...
                try {
                    ...
                    if (response.isSuccessful()) {
                        AddBillItem.this.taxReceivedList = response.body().getData();
                    }
                } catch (Exception e) {
    ...
    /*public void showtaxs(List<ProductServiceTaxesItem> list) {
        try {
            ...
            btn_apply_tax.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    dialog.dismiss();
                    ListPurchaseItem.adds_item.setProductServiceTaxes(ta.getItemList());
                    spinner_taxtes.setText("Taxts + " + ListPurchaseItem.adds_item.getProductServiceTaxes().size());
                }
            });
            dialog.show();
        } catch (Exception e) {
        ...
        try {
            ...
            btn_apply_tax.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    try {
                        taxs=ta.getItemList();
                        ListPurchaseItem.adds_item.setProductServiceTaxes(taxs);
                        spinner_taxtes.setText("Taxts + " + ListPurchaseItem.adds_item.getProductServiceTaxes().size());
                    } catch (Exception e) {
